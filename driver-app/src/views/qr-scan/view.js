...
export async function mount(rootElement) {
  ...
  const verificationTextEl = rootElement.querySelector(
  ...
  try {
    ...
    currentOrder = orders.find(
      (o) => String(o?.OrderID) === String(expectedOrderId),
    );

    // 3. Populate Initial View
    if (parcelIdEl)
      parcelIdEl.textContent = `Order #${currentOrder?.OrderID || expectedOrderId}`;
    if (customerNameEl)
      customerNameEl.textContent = `Recipient: ${currentOrder?.customer?.user?.name || "N/A"}`;
  ...
  async function initCamera() {
    ...
    try {
      ...
      if (verificationTextEl)
        verificationTextEl.textContent = "CAMERA ERROR: ACCESS DENIED";
    }
  }
